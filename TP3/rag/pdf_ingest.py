...
def load_and_split_resume(path: str, person_id: str, person_name: str) -> list[Document]:
    """Load PDF and split into chunks with person metadata."""
    logger.info(f"Loading resume for {person_name} from {path}...")
    ...
    try:
        ...
        logger.info(f"Loaded {len(docs)} pages from {person_name}'s resume.")
    except Exception as e:
        logger.error(f"Failed to load PDF for {person_name}: {e}")
        raise
    ...
    # ADD PERSON METADATA to each chunk
    ...
    logger.info(f"Split into {len(split_docs)} chunks for {person_name}.")
    return split_docs
...
def ingest_resume(force_reingest: bool = False):
    """
    Ingest ALL resumes into Pinecone with person metadata.
    
    Args:
        force_reingest: If True, clear existing vectors and re-ingest.
    """
    try:
        logger.info("Starting multi-resume ingestion process...")
        ...
        for person_id, info in PERSONS.items():
            person_docs = load_and_split_resume(
                path=info["file"],
                person_id=person_id,
                person_name=info["name"]
            )
            all_docs.extend(person_docs)
        
        logger.info(f"Total chunks to ingest: {len(all_docs)}")
        ...
        logger.info(f"Ingesting {len(all_docs)} chunks in {total_batches} batch(es)...")
...
if __name__ == "__main__":
    print("=" * 60)
    print("Starting Multi-Resume Ingestion Script")
    print("=" * 60)
