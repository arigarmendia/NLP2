...
class Trainer():
    ...
    def __init__(self,
        ...
        if self.device == 'mps' and not torch.backends.mps.is_available():
            raise RuntimeError("MPS backend not available. Ensure PyTorch is installed with MPS support.")
    ...
    def train_model_v2(self, use_amp: bool = False, dtype: torch.dtype = torch.bfloat16):
        ...
        if self.device == 'mps' and dtype == torch.float16:
            warnings.warn("On MPS, prefer torch.bfloat16 over torch.float16 to avoid autocast warnings.")

        model = self.model.train()
        self.scaler = torch.amp.GradScaler(device=self.device, enabled=use_amp)  # Store scaler as self.scaler for checkpointing
        losses = []
        ...
        for train_input, train_mask in bar:
            ...
            losses.append(loss.item())  # Append scalar

            loss = loss / self.gradient_accumulation_steps  # Tensor for backward
            ...
            if len(losses) % 10 == 0:
                bar.set_description(f"loss {np.mean(losses[-10:]):.5f}")
        ...
        torch.mps.empty_cache() if self.device == 'mps' else torch.cuda.empty_cache()  # MPS-specific empty cache
        return np.mean(losses[-10:]) if len(losses) >= 10 else np.mean(losses)
